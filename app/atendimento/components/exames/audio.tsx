...
const Audiometria: React.FC<AudiometriaProps> = ({
  ...
  const [formData, setFormData] = useState<AudiometriaData>({
    ...
    classificacaoOD: "Audição Normal",
    classificacaoOE: "Audição Normal",
  ...
  const identificarFrequenciasAlteradas = useCallback(
    (vaLimiares: { [key: number]: string }): string => {
  ...
  const verificarCriterioPCD = useCallback(
    (mediaOD: number, mediaOE: number): string => {
      const melhorOrelha = Math.min(mediaOD, mediaOE);
      const piorOrelha = Math.max(mediaOD, mediaOE);

      let criterioPCD =
        "NÃO SE ENQUADRA. Limiares auditivos abaixo dos critérios federais (Média Tonal na Melhor Orelha < 41 dB NA).";
  ...
  const calcularIndicesFala = useCallback((data: AudiometriaData) => {
    ...
    const calcularIRFDissimetrica = (irfOD: number, irfOE: number) => {
      if (isNaN(irfOD) || isNaN(irfOE) || irfOD === 0 || irfOE === 0)
        return "Não avaliado";
    ...
    const srtODCalculado = calcularSRT(
      data.viaAereaOD500,
      data.viaAereaOD1000,
      data.viaAereaOD2000,
    );
    const srtOECalculado = calcularSRT(
      data.viaAereaOE500,
      data.viaAereaOE1000,
      data.viaAereaOE2000,
    );
    ...
    return {
      ...
      srtOD:
        data.srtOD ||
        (srtODCalculado.srt > 0 ? srtODCalculado.srt.toString() : ""),
      resultadoSRTOD: srtODCalculado.resultado,
      srtOE:
        data.srtOE ||
        (srtOECalculado.srt > 0 ? srtOECalculado.srt.toString() : ""),
  ...
  type NR7Result =
    ...
    | "Desvio Padrão (DPLA)"
    | "Confirmação de PAIR (Anexo II, NR-7)"
    | "Protocolo NR-7 Incompleto"
    | "Audição Normal";
  const classificarDPLA = useCallback(
    (
      ...
      if (Object.keys(referenceVA).length === 0)
        return "Protocolo NR-7 Incompleto";

      const media4f = calcularMediaTonal([
        currentVA[500],
        currentVA[1000],
        currentVA[2000],
        currentVA[3000],
  ...
  const vaInputs = useMemo(
    () => [
      formData.viaAereaOD250,
      formData.viaAereaOD500,
      formData.viaAereaOD1000,
      formData.viaAereaOD2000,
      formData.viaAereaOD3000,
      formData.viaAereaOD4000,
      formData.viaAereaOD6000,
      formData.viaAereaOD8000,
      formData.viaAereaOE250,
      formData.viaAereaOE500,
      formData.viaAereaOE1000,
      formData.viaAereaOE2000,
      formData.viaAereaOE3000,
      formData.viaAereaOE4000,
      formData.viaAereaOE6000,
      formData.viaAereaOE8000,
    ],
    [
      formData.viaAereaOD250,
      formData.viaAereaOD500,
  ...
  useEffect(() => {
    ...
    const frequenciasAlteradasOD =
      identificarFrequenciasAlteradas(vaLimiaresOD);
    const frequenciasAlteradasOE =
      identificarFrequenciasAlteradas(vaLimiaresOE);
    ...
    setFormData((prev) => {
      ...
      for (const key in updates) {
        if (
          prev[key as keyof AudiometriaData] !==
          updates[key as keyof AudiometriaData]
        ) {
          hasChanged = true;
          break;
        }
  ...
  const frequencias = [
    {
      ...
      fieldVOOD: null,
      fieldVOOE: null,
      fieldMascOD: "mascaramentoOD250",
      fieldMascOE: "mascaramentoOE250",
    },
    {
      label: "500 Hz",
      ...
      fieldMascOD: "mascaramentoOD500",
      fieldMascOE: "mascaramentoOE500",
    },
    {
      label: "1000 Hz",
      ...
      fieldMascOD: "mascaramentoOD1000",
      fieldMascOE: "mascaramentoOE1000",
    },
    {
      label: "2000 Hz",
      ...
      fieldMascOD: "mascaramentoOD2000",
      fieldMascOE: "mascaramentoOE2000",
    },
    {
      label: "3000 Hz",
      ...
      fieldMascOD: "mascaramentoOD3000",
      fieldMascOE: "mascaramentoOE3000",
    },
    {
      label: "4000 Hz",
      ...
      fieldMascOD: "mascaramentoOD4000",
      fieldMascOE: "mascaramentoOE4000",
    },
    {
      label: "6000 Hz",
      ...
      fieldVOOD: null,
      fieldVOOE: null,
      fieldMascOD: "mascaramentoOD6000",
      fieldMascOE: "mascaramentoOE6000",
    },
    {
      label: "8000 Hz",
      ...
      fieldVOOD: null,
      fieldVOOE: null,
      fieldMascOD: "mascaramentoOD8000",
      fieldMascOE: "mascaramentoOE8000",
    },
  ];

  // Handlers para navegação com TAB
  const handleKeyDownVAOD = useCallback(
    (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.key === "Tab") {
        ...
        if (
          nextIndex < inputRefsVAOD.current.length &&
          inputRefsVAOD.current[nextIndex]
        ) {
  ...
  const handleKeyDownVAOE = useCallback(
    (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.key === "Tab") {
        e.preventDefault();
        const nextIndex = index + 1;
        ...
        if (
          nextIndex < inputRefsVAOE.current.length &&
          inputRefsVAOE.current[nextIndex]
        ) {
          inputRefsVAOE.current[nextIndex]?.focus();
        } else if (inputRefsVOOD.current.length > 0) {
          const firstVOODIndex = inputRefsVOOD.current.findIndex(
            (ref) => ref !== null,
          );
  ...
  const handleKeyDownVOOD = useCallback(
    (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.key === "Tab") {
        e.preventDefault();
        const nextIndex = index + 1;
        ...
        if (
          nextIndex < inputRefsVOOD.current.length &&
          inputRefsVOOD.current[nextIndex]
        ) {
          inputRefsVOOD.current[nextIndex]?.focus();
        } else if (inputRefsVOOE.current.length > 0) {
          const firstVOOEIndex = inputRefsVOOE.current.findIndex(
            (ref) => ref !== null,
          );
  ...
  const handleKeyDownVOOE = useCallback(
    (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.key === "Tab") {
        e.preventDefault();
        const nextIndex = index + 1;
        ...
        if (
          nextIndex < inputRefsVOOE.current.length &&
          inputRefsVOOE.current[nextIndex]
        ) {
  ...
  const handleKeyDownMascOD = useCallback(
    (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.key === "Tab") {
        e.preventDefault();
        const nextIndex = index + 1;
  ...
  const handleKeyDownMascOE = useCallback(
    (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.key === "Tab") {
        e.preventDefault();
        const nextIndex = index + 1;
    ...
    <div className="space-y-8 p-4 md:p-8  min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {" "}
                  Queixa ou sintoma auditivo atual?{" "}
                </label>
                  ...
                  {["Sim", "Não"].map((option) => (
                    <Checkbox
                      ...
                      onValueChange={(checked) =>
                        checked &&
                        handleCheckboxChange("queixaAuditiva", option)
                      }
                    >
                      {option}
                    </Checkbox>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {" "}
                  Infecção ou Cirurgia no Ouvido?{" "}
                </label>
                <div className="flex gap-4">
                  {["Sim", "Não"].map((option) => (
                    <Checkbox
                      ...
                      onValueChange={(checked) =>
                        checked &&
                        handleCheckboxChange("infeccaoCirurgiaOuvido", option)
                      }
                    >
                      {option}
                    </Checkbox>
              ...
              <div className=" p-4 rounded-lg col-span-full">
                <h3 className="font-bold text-gray-700 mb-3 text-sm uppercase tracking-wide text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      {" "}
                      Já trabalhou em ambiente ruidoso?{" "}
                    </label>
                    <div className="flex gap-4">
                      <Checkbox
                        classNames={{ label: "text-gray-700" }}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      {" "}
                      Trabalha atualmente em ambiente ruidoso?{" "}
                    </label>
                    <div className="flex gap-4">
                      <Checkbox
                        classNames={{ label: "text-gray-700" }}
              ...
              <div className=" p-4 rounded-lg col-span-full">
                <h3 className="font-bold text-center text-gray-700 mb-3 text-sm uppercase tracking-wide">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <div className="flex gap-4">
                      <Checkbox
                        classNames={{ label: "text-gray-700" }}
                        isSelected={formData.traumaCabecaOuvido === "Sim"}
                        onValueChange={(checked) =>
                          handleCheckboxChange(
                            "traumaCabecaOuvido",
                            checked ? "Sim" : "Não",
                      ...
                      <Checkbox
                        classNames={{ label: "text-gray-700" }}
                        isSelected={formData.traumaCabecaOuvido === "Não"}
                        onValueChange={(checked) =>
                          handleCheckboxChange(
                            "traumaCabecaOuvido",
                            checked ? "Não" : "Sim",
            ...
            <div className="grid grid-cols-3 md:grid-cols-3 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange("dataCalibracao", e.target.value)
                  }
                />
              </div>
              <div>
          ...
          <div className="p-4 ">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="md:col-span-1">
                ...
                <Input
                  className="border-gray-300 bg-white"
                  placeholder="Ex: Cerúmen impactado OD"
          ...
          <table className="w-full border-collapse border border-gray-300">
            <thead>
              ...
              <tr className="bg-gray-50">
                {frequencias.map((freq) => (
                  <th
                    key={freq.label}
                    className="border border-gray-300 p-2 font-medium text-sm text-center"
                  >
                    {freq.label}
                  </th>
                ))}
            ...
            <tbody>
              ...
              <tr className="bg-white">
                <td
                  className="border border-gray-300 p-2 text-center font-medium text-red-700 bg-red-50"
                  rowSpan={3}
                >
                  {" "}
                  OD{" "}
                ...
                {frequencias.map((freq, index) => (
                  ...
                  >
                    <Input
                      ref={(el) => {
                        inputRefsVAOD.current[index] = el;
                      }}
                ...
                {frequencias.map((freq, index) => (
                  ...
                  >
                    {freq.fieldVOOD ? (
                      <Input
                        ref={(el) => {
                          inputRefsVOOD.current[index] = el;
                        }}
                        className="text-center border-red-300 bg-white focus:border-red-500 h-10 w-full"
                        max="120"
                        min="0"
              ...
              <tr className="bg-white">
                ...
                {frequencias.map((freq, index) => (
                  <td
                    key={`masc-od-${freq.label}`}
                    className="border border-gray-300 p-1"
                  >
                    <Input
                      ref={(el) => {
                        inputRefsMascOD.current[index] = el;
                      }}
                      className="text-center border-red-300 bg-white focus:border-red-500 h-10 w-full"
                      max="120"
                      min="0"
                      ...
                      value={
                        formData[
                          freq.fieldMascOD as keyof AudiometriaData
                        ] as string
                      }
                      onChange={(e) =>
                        handleDecibelInputChange(
              ...
              <tr className="bg-white">
                <td
                  className="border border-gray-300 p-2 text-center font-medium text-blue-700 bg-blue-50"
                  rowSpan={3}
                >
                  {" "}
                  OE{" "}
                ...
                {frequencias.map((freq, index) => (
                  ...
                  >
                    <Input
                      ref={(el) => {
                        inputRefsVAOE.current[index] = el;
                      }}
                ...
                {frequencias.map((freq, index) => (
                  ...
                  >
                    {freq.fieldVOOE ? (
                      <Input
                        ref={(el) => {
                          inputRefsVOOE.current[index] = el;
                        }}
                        className="text-center border-blue-300 bg-white focus:border-blue-500 h-10 w-full"
                        max="120"
                        min="0"
              ...
              <tr className="bg-white">
                ...
                {frequencias.map((freq, index) => (
                  <td
                    key={`masc-oe-${freq.label}`}
                    className="border border-gray-300 p-1"
                  >
                    <Input
                      ref={(el) => {
                        inputRefsMascOE.current[index] = el;
                      }}
                      className="text-center border-blue-300 bg-white focus:border-blue-500 h-10 w-full"
                      max="120"
                      min="0"
                      ...
                      value={
                        formData[
                          freq.fieldMascOE as keyof AudiometriaData
                        ] as string
                      }
                      onChange={(e) =>
                        handleDecibelInputChange(
      ...
      <Card className="p-6 shadow-sm border border-gray-200 bg-white">
        <SectionTitle
          ...
          number="4"
          title="Índices de Reconhecimento de Fala (Opcional)"
        />
        ...
        <div className="flex items-center gap-2 mb-6">
          <Checkbox
            ...
            onValueChange={(checked) =>
              handleInputChange("realizarIRF", checked)
            }
          >
        ...
        {formData.realizarIRF && (
          <div className="space-y-6">
            ...
            <div className="p-4 ">
              <h3 className="font-semibold text-gray-700 mb-3 text-sm uppercase tracking-wide">
                SRT - Speech Reception Threshold (Limiar de Reconhecimento de
                Fala)
              </h3>
              <p className="text-xs text-gray-500 mb-4">
                O valor do SRT é calculado automaticamente com base na média de
                500, 1000 e 2000 Hz, mas pode ser preenchido manualmente.
              </p>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleDecibelInputChange("srtOD", e.target.value)
                        }
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        className={`text-center font-bold bg-transparent border-none ${
                          formData.resultadoSRTOD === "Normal"
                            ? "text-green-600"
                            : formData.resultadoSRTOD === "Limítrofe"
                              ? "text-amber-500"
                              : "text-red-600"
                        }`}
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleDecibelInputChange("srtOE", e.target.value)
                        }
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        isReadOnly
                        className={`text-center font-bold bg-transparent border-none ${
                          formData.resultadoSRTOE === "Normal"
                            ? "text-green-600"
                            : formData.resultadoSRTOE === "Limítrofe"
                              ? "text-amber-500"
                              : "text-red-600"
                        }`}
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleInputChange("irfOD", e.target.value)
                        }
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleDecibelInputChange("irfDBOD", e.target.value)
                        }
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        isReadOnly
                        className={`text-center font-bold bg-transparent border-none ${
                          formData.resultadoIRFMonoauralOD.includes("Normal")
                            ? "text-green-600"
                            : formData.resultadoIRFMonoauralOD.includes(
                                  "Limítrofe",
                                )
                              ? "text-amber-500"
                              : "text-red-600"
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleInputChange("irfOE", e.target.value)
                        }
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleDecibelInputChange("irfDBOE", e.target.value)
                        }
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        isReadOnly
                        className={`text-center font-bold bg-transparent border-none ${
                          formData.resultadoIRFMonoauralOE.includes("Normal")
                            ? "text-green-600"
                            : formData.resultadoIRFMonoauralOE.includes(
                                  "Limítrofe",
                                )
                              ? "text-amber-500"
                              : "text-red-600"
      ...
      <Card className="p-6 shadow-sm border border-gray-200 bg-white">
        ...
        <div className="space-y-6">
          ...
          {/* Conclusão Geral */}
          <div className="p-4">
            <h3 className="font-bold text-gray-700 mb-3 text-md uppercase tracking-wide">
              Resultados
            </h3>
