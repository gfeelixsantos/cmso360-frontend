...
const handleExameSelecionado = (exame: string) => {
  ...
  if(setList) setCodigosDeAtendimento(setList)
...
const loadInitialTickets = async () => {
  ...
  const getSchedulings = async () => {
    try {
      ...
      // atendimento geral
      const schedulesFiltred = schedulesToUnit.filter(s => !agendamentos.some(a => a._id === s._id))
  ...
  useEffect(() => {
    ...
    const meusAtendimentos = emAtendimento.filter(atend => 
      atend.EXAMES.some(exame => codigosDeAtendimento.has(exame.codigoExame) && exame.status === ExamStatus.PENDENTE ));

    if(meusAtendimentos) setAgendamentos(meusAtendimentos)
  },[agendamentosGeral])
