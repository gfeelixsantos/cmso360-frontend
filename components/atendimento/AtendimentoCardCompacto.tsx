...
const getStatusColor = (status: string) => {
  switch (status) {
    case TicketStatus.AGUARDANDO: return "bg-green-100 text-green-800";
    case TicketStatus.EM_CHAMADA: return "bg-amber-100 text-amber-800";
    case TicketStatus.EM_ATENDIMENTO: return "bg-red-100 text-red-800";
    case TicketStatus.FINALIZADO: return "bg-gray-100 text-gray-800";
...
// Componente compacto
const AtendimentoCardCompacto: React.FC<AtendimentoCardProps> = ({ atendimento }) => {
  const { border, bg } = getStatusVisual(atendimento.TICKET.status);
  const { progress, completed, total } = useExamProgress(atendimento.EXAMES || []);
  ...
  return (
    <Card className={`${bg} ${border} rounded-lg p-3 mb-2 hover:shadow-md transition-shadow cursor-pointer`}>
