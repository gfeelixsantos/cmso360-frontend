...
const AtendimentoContent: React.FC<MainContentProps> = ({
  ...
  const naoEstaEmOutrasSalas = useMemo(() => 
    (atendimento: Scheduling) => !prontuariosEmAtendimento.has(atendimento.CODIGOPRONTUARIO),
    [prontuariosEmAtendimento]
  );

  const [senhasPreferenciais, senhasComPrefixo, senhasNormais] = useMemo(() => {
    const preferenciais = AtendimentosOrdenados.filter((s) => 
      s.TICKET?.preferencial && naoEstaEmOutrasSalas(s)
    );
    const comPrefixo = AtendimentosOrdenados.filter((s) => 
      (s.TICKET?.prefixo && !s.TICKET?.preferencial) && naoEstaEmOutrasSalas(s)
    );
    const normais = AtendimentosOrdenados.filter((s) => 
      (!s.TICKET?.preferencial && !s.TICKET?.prefixo) && naoEstaEmOutrasSalas(s)
    );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-default-50/50">
        <div className="text-center space-y-6">
          ...
          {/* Progresso indeterminado do HeroUI */}
          <div className="w-48 mx-auto">
            <Progress
              ...
              classNames={{
                ...
                indicator: "bg-gradient-to-r from-green-700 to-green-300"
              }}
            />
          </div>
        </div>
