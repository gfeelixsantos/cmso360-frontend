...
class ClinicaCalculator {
  ...
  static calcularIMC(peso: string, altura: string): { imc: string; resultado: string } {
...
const FichaClinicaOcupacional: React.FC<FichaClinicaProps> = ({ 
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      const { imc, resultado } = ClinicaCalculator.calcularIMC(formData.peso, formData.altura);
  ...
  const handlePressaoArterialChange = useCallback((index: number, value: string) => {
  ...
  const AptidaoCheckboxGroup = useCallback(({ 
    ...
    <div className="p-4 grid grid-cols-2">
      <span className="text-sm align-center font-medium text-gray-700">{label}</span>
      <div className="flex gap-6">
        ...
        <div>
          ...
          <Card className="p-6 shadow-none border border-gray-200 bg-white mb-6">
            ...
            <div className="space-y-4">
              <AptidaoCheckboxGroup field="trabalhoAltura" label="Trabalho em altura" />
              <AptidaoCheckboxGroup field="trabalhoEspacoConfinado" label="Trabalho em espaço confinado" />
              <AptidaoCheckboxGroup field="capacidadeCarregarPeso" label="Capacidade para carregar peso" />
              <AptidaoCheckboxGroup field="aptoOperarVeiculos" label="Apto a operar veículos" />
            </div>
          </Card>
          ...
          <Card className="p-6 shadow-none border border-gray-200 bg-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <ExameClinicoItem label="Cabeça / Pescoço" field="cabecaPescoco" />
              <ExameClinicoItem label="Tórax" field="torax" />
              <ExameClinicoItem label="Abdome" field="abdome" />
              <ExameClinicoItem label="Coluna" field="coluna" />
              <ExameClinicoItem label="Membros Superiores" field="membrosSuperiores" />
              <ExameClinicoItem label="Membros Inferiores" field="membrosInferiores" />
            </div>
            ...
            {(atendimento?.SEXO === 'Feminino' || atendimento?.TIPOEXAMENOME === "DEMISSIONAL") && (
              <div className="p-4">
                ...
                <FormattedInput
                  ...
                  onChange={(value) => handleInputChange('ultimaMenstruacao', value)}
      ...
      <Card className="p-6 shadow-none border border-gray-200 bg-white">
        ...
        <div className="p-4">
          {(Array.isArray(formData.pressaoArterial) ? formData.pressaoArterial : []).map((pa, index) => (
            <div key={index} className="grid grid-cols-1 md:grid-cols-3 gap-3 align-center items-center bg-white mb-3">
              <div>
                <Input
                  variant='bordered'
                  label={"Valor (mmHg)"}
                  value={pa.valor}
                  onChange={(value) => handlePressaoArterialChange(index, value.target.value)}
                  placeholder="000/00"
                  maxLength={7}
                />
              </div>
              ...
              <div>
                ...
                {pa.valor && ClinicaCalculator.verificarPressaoAlterada(pa.valor) && (
                  <div className='flex mt-2 gap-2'>
                    ...
                    <p className="text-xs text-red-600 font-medium mt-1">Pressão arterial alterada</p>
                  </div>
                )}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 ">
          <div className="p-4">
            <Input
              ...
              label="Peso (kg)"
              value={formData.peso}
              onChange={(value) => handleInputChange("peso", value.target.value)}
              placeholder="00,0"
              type="text"
              className={formErrors.peso ? 'border-red-500' : ''}
            />
            {formErrors.peso && <p className="text-xs text-red-600 mt-1">{formErrors.peso}</p>}
          </div>
          ...
          <div className="p-4">
            <Input
              ...
              onChange={(value) => handleInputChange("altura", value.target.value)}
              placeholder="0,00"
              type="text"
              className={formErrors.altura ? 'border-red-500' : ''}
            />
            <p className="text-xs text-gray-500 mt-1">Vírgula automática</p>
            {formErrors.altura && <p className="text-xs text-red-600 mt-1">{formErrors.altura}</p>}
          </div>
          ...
          <div className="p-4">
            ...
            {formData.resultadoImc && (
              <p className={`text-xs font-medium mt-1 ${
                formData.resultadoImc.includes("normal") ? "text-green-600" :
                formData.resultadoImc.includes("Abaixo") ? "text-amber-600" : "text-orange-600"
              }`}>
                {formData.resultadoImc}
              </p>
            )}
