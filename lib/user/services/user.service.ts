...
export class UserService {
     ...
     */
    static async login(user: IUserLogin): Promise<ApiResponse<{ token:string, userInfo: IUserInfo }>> {

        // Busca usuário por CPF no Supabase
        const userRegister = await SupabaseService.getUserByCpf(user.cpf)
        
        if (!userRegister) {
            return new ApiResponse(HttpCodes.BAD_REQUEST, ApiMessages.USER_INPUT_INVALID)
        }

        // Valida senha utilizando bcrypt
        const passwordIsValid = await Bcrypt.comparePasswords(user.password, userRegister.password)
        if (!passwordIsValid) {
            return new ApiResponse(HttpCodes.UNAUTHORIZED, ApiMessages.USER_INPUT_INVALID)
        }

        // Busca informações complementares no SOC
        const cadastroPessoas = await SOC.ExportaDadosCadastroPessoas()
        const socUser = cadastroPessoas?.find(p => p.CODIGO == userRegister.codigo)

        if (!socUser) {
            return new ApiResponse(HttpCodes.UNPROCESSABLE_ENTITY, ApiMessages.SOC_ED_CADASTRO_PESSOAS_NULL)
        }
        ...
        return new ApiResponse(HttpCodes.OK, ApiMessages.USER_LOGGED_IN_SUCCESSFULLY, { token: token, userInfo: userInfo })
    }
    ...
    static async register(user: IUserRegister) {   
        try {
            ...
            if (!cadastroPessoas) {
                return new ApiResponse(HttpCodes.INTERNAL_SERVER_ERROR, ApiMessages.SOC_ED_CADASTRO_PESSOAS_NULL, null)
            }

            if (supabaseData?.cpf) {
                return new ApiResponse(HttpCodes.CONFLICT, ApiMessages.USER_ALREADY_EXISTS, null)
            }
            ...
            if (!socRegisterUser) {
                return new ApiResponse(HttpCodes.NOT_FOUND, ApiMessages.SOC_CADASTRO_PESSOA_NOT_FOUND, null)
            }
            ...
            if (statusCode != HttpCodes.CREATED) {
                return new ApiResponse(HttpCodes.UNPROCESSABLE_ENTITY, ApiMessages.USER_REGISTER_FAILED, null)
            }

            // Mapeia para DTO de resposta
            const userInfoMapped = mapCadastroPessoasToUserInfo(socRegisterUser)
            return new ApiResponse(HttpCodes.CREATED, ApiMessages.USER_REGISTERED_SUCCESSFULLY, userInfoMapped)
        } 
        catch (err) {
            console.error(err)
            return new ApiResponse(HttpCodes.INTERNAL_SERVER_ERROR, ApiMessages.INTERNAL_ERROR, null)
        }
    }
}
